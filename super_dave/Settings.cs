...
public class Settings {
    ...
    // Diving
    public static ConfigEntry<bool> m_auto_pickup_debug_mode;
    ...
    public void load(DDPlugin plugin) {
        ...
        // Diving
        m_auto_pickup_debug_mode = this.m_plugin.Config.Bind<bool>("Diving", "Diving - Auto-pickup: Debug Mode", false, "Set to true to echo item pickup messages to the LogOutput.log file for debugging which items to un-comment in auto-pickup-items.txt file.");
        ...
        m_toxic_aura_enabled = this.migrate_option<bool>("Diving", "Toxic Aura: Enabled", "Diving - Toxic Aura: Enabled", false, "Set to true to enable the instant-fish-killing (or sleeping if 'Toxic Aura: Sleep Effect' is true [default]) aura around Dave.");
        m_toxic_aura_sleep = this.m_plugin.Config.Bind<bool>("Diving", "Diving - Toxic Aura: Sleep Effect", true, "Set to false to switch from sleep aura to kill aura.");
        m_aura_radius = this.migrate_option<float>("Diving", "Toxic Aura: Radius", "Diving - Toxic Aura: Radius", 6.0f, "Radius (in meters?) around the character in which fish will be insta-killed, if Toxic Aura is enabled (float, default 6.0f).");
